...
public class DateTextProcessor {
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
        ...
        if (rowData == null) {
            if (accumulatedText.length() > 0) {
                outputRow(currentDate, accumulatedText.toString().trim());
            }
            setOutputDone();
            return false;
        }
    ...
    private void outputRow(String date, String text) throws KettleException {
        Object[] outputRow = createOutputRow(null, data.outputRowMeta.size());
        get(Fields.Out, "date").setValue(outputRow, date);
        get(Fields.Out, "text").setValue(outputRow, text);
